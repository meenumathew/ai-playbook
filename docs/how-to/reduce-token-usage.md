...
### 1. Measure the static surface

```bash
ai-playbook context-report --agent all
ai-playbook context-report --agent xp-pair-programmer --json
```

Record this before and after changing agents, preloads, packs, or the
always-loaded rules. The report uses a provider-neutral characters/4 estimate;
it is useful for relative comparison, not billing. Conversation history, tool
schemas, cache behaviour, and files loaded later require provider-native
telemetry.

### 2. Deploy less surface
...
### 3. Use the built-in loading discipline
...
### 4. Keep the always-loaded prefix stable
...
### 5. Match the model tier to the step

Map `executor` tier to a fast model for the edit/test loop and reserve the stronger `advisor` model for refining, planning, and review: see `knowledge-base/model-tier.md` § Capability Mapping. Compare `context-report` before/after values for static playbook cost, and use provider-native usage reports for actual tokens and cache behaviour. The optional local telemetry log deliberately omits model and token details.

### 6. Use the tier and lane switches for low-stakes work
...
### 7. Enforce read budgets by mechanism (Claude Code, opt-in)
...
### 8. Practice session hygiene (the biggest lever the playbook cannot automate)
...
### 9. Spend subagents deliberately
...
Before fan-out, confirm the tasks are independent: no task should require
another task's unfinished output, and concurrent writers must not have
overlapping writes. Otherwise, keep the work sequential. Give each subagent a
self-contained brief with its objective, bounded scope, relevant paths,
read-only or write authority, expected result, and verification requirement.

One integration owner reviews the returned evidence and combined diff. Do not
concatenate outputs on trust. Resolve contradictions explicitly, then run the
integrated verification for the repository state that will actually be kept.

### 10. Avoid the common waste patterns
...
| Never clearing between tasks | Yesterday's story taxes today's turns | § 8 session hygiene: story boundary = `/clear` |
