...
from deploy_ai_playbook.cli import VERSION_FILE, Tool, write_version_file
from deploy_ai_playbook.telemetry import deploy_telemetry_hook_config
from tests.acceptance._dsl import deploy, disable, doctor

RUNTIME_DIRS = ("stories", "plans", "research", "audits", "reviews", "incidents")


def test_doctor_nothing_deployed_exits_with_error(tmp_path: Path):
...
def test_doctor_json_reports_not_deployed(tmp_path: Path):
    result = doctor(tmp_path, as_json=True)
...
def test_doctor_detects_missing_agent(deployed_claude: Path):
    ...
    result = doctor(deployed_claude)

    assert result.exit_code == 0
    assert "not deployed" in result.output


def test_doctor_respects_partial_deployment_scope(tmp_path: Path):
    ...
    assert result.exit_code == 0
    assert "Agent slice-planner is not deployed" not in result.output
    assert "Harness file" not in result.output
...
def test_doctor_respects_deployed_language_filter(tmp_path: Path):
    """Doctor must not report KB files omitted by deploy --language as missing."""
    deploy_result = deploy(tmp_path, language="python", mcp=False)
    assert deploy_result.exit_code == 0, deploy_result.output
...
def test_doctor_json_reports_health_status(deployed_claude: Path):
    ...
    result = doctor(deployed_claude, as_json=True)
...
def test_doctor_warns_when_tool_flag_disagrees_with_deployed_tool(deployed_claude: Path):
    ...
    result = doctor(deployed_claude, tool="copilot")
...
def test_doctor_warns_when_codex_telemetry_session_end_hook_missing(tmp_path: Path):
    result = deploy(tmp_path, tool="codex", mcp=False)
    assert result.exit_code == 0, result.output
    (tmp_path / ".codex" / "hooks.json").unlink()

    result = doctor(tmp_path, tool="codex")
...
def test_doctor_strict_exits_2_when_not_deployed(tmp_path: Path):
    ...
    result = doctor(tmp_path, strict=True)
...
def test_doctor_strict_exits_1_when_issues_found(deployed_claude: Path):
    ...
    result = doctor(deployed_claude, strict=True)
    assert result.exit_code == 1
    assert "not deployed" in result.output
...
def test_doctor_default_exit_unchanged_for_issues(deployed_claude: Path):
    """Default doctor behaviour (no --strict) preserves the original 0-on-issues contract.

    Adopters relying on the original contract: issues print but exit 0: must
