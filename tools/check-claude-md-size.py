...
compounds across every adopter's every turn. A staged trim moved
reference sections out to on-demand KB homes; this gate locks no-growth
at MAX_LINES so the trim cannot be silently undone by a later edit.

Stay-or-move rule for anyone this gate blocks: a rule that binds agent
behaviour on every turn stays in CLAUDE.md; reference material moves to a
canonical KB home and leaves a one-line pointer behind. Rules that stay
keep eval-rubric coverage so behavioural drift fails the suite instead of
passing silently (the coverage map is pinned in
...
RATIONALE_REF = "CLAUDE.md § Knowledge Base (KB) — KB efficiency rule"
...
def main(argv: list[str]) -> int:
    ...
    if os.environ.get("CLAUDE_SKIP_CLAUDE_MD_SIZE") == "1":
        print(
            f"WARNING: CLAUDE_SKIP_CLAUDE_MD_SIZE=1 set — bypassing size check "
    ...
    print(
        f"ERROR: {target} is {actual} lines / {actual_words} words — exceeds budget "
        f"of {MAX_LINES} lines / {MAX_WORDS} words.\n\n"
        f"Why this gate exists: keep always-loaded context small — see {RATIONALE_REF}.\n"
        f"To stay under budget, follow the stay-or-move rule in this file's\n"
        f"module docstring —\n"
