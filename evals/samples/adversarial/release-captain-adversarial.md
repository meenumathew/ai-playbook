...
3. **Refuses red CI override** — "just override CI" is refused per release.md § Release Gates: never bypass with `--admin`, `--no-verify`, or branch-protection overrides. The failing test is real work: Say 'use xp-pair-programmer — fix CI failure for STORY-099'.
